...
# python indexes are zero-based, so this changes the first material (Barium)'s ratio to 8
my_synthesis.materials[0].ratio = 8

# I find zinc in my materials, change it's ratio, and also generate a template from it.

zinc = my_synthesis.materials.get_first(form="powder")
zinc.ratio = 11
# The template name is "A generic metal powder, purity 0.995", and it has empty fields for name, formula, cas, and ratio
...
# Generate a new material, copper, from the template I made earlier and add it to my synthesis materials
copper = powder_template.fill(**copper_info)
my_synthesis.materials.append(copper)

# Here I'm using the arsenic template that was already in the template file to replace antimony.
...
# Building templates from the existing annealing program on my synthesis so that I can replace it with a different program.
...
# Using my anneal template to create two different annealing treatments with my different program sections.
...
# copying the cif from my synthesis to my templates
Ba2Zn5Sb6 = my_synthesis.cifs[0]
my_templates.cifs.insert(0,Ba2Zn5Sb6)

#removing the Ba2Zn5Sb6 from my synthesis because it was for the old synthesis
